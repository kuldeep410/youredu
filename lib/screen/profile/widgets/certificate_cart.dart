...
class CertificateCart extends StatelessWidget {
  final UserCertificateResponse? userCertificateResponse;

  const CertificateCart({Key? key, this.userCertificateResponse})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    return userCertificateResponse?.data?.certificates == null
        ? Column(
            children: [
              SizedBox(
                height: 20.h,
        ...
        : ListView.builder(
            ...
            itemCount: userCertificateResponse?.data?.certificates?.length ?? 0,
            itemBuilder: (context, index) {
              final data = userCertificateResponse?.data?.certificates?[index];
              return Column(
