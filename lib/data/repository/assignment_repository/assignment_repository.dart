...
class AssignmentRepository {
  static Future<ApiResponse<AssignmentResponse>>
      getAssignmentRepositoryData() async {
    try {
      EasyLoading.show(status: 'loading...');
      var response = await ApiService.getDio()!.get("/student/assignment");
      EasyLoading.dismiss();
      if (response.statusCode == 200) {
        if (kDebugMode) {
          print(response.data);
        }
        var obj = assignmentResponseFromJson(response.toString());
        return ApiResponse(
      ...
      if (e.type == DioExceptionType.unknown) {
        EasyLoading.dismiss();
        var obj = assignmentResponseFromJson(e.response.toString());
